...
class Order extends Base
{
    ...
    public function ajaxindex()
    {
        ...
        $pay_begin = strtotime(I('pay_time_begin'));
        $pay_end = strtotime(I('pay_time_end'));
        // 搜索条件
        $condition = array();
        $keyType = I("keytype");
        ...
        } elseif ($is_integral == 2) {
            $condition['integral'] = 0;
        }
        $is_drug = I('is_drug',0);
        ...
        if ($pay_begin && $pay_end) {
            $condition['pay_time'] = array('between', "$pay_begin,$pay_end");
        }
        ...
        $condition['status'] = 1;
		// $condition['user_id'] = ['neq',194836];
        $sort_order = I('order_by', 'DESC') . ' ' . I('sort');
        $count = M('order')->where($condition)->count();
        if ($is_integral == 2) {
            $sum_integral = 0;
        } else {
            $sum_integral = M('order')->where($condition)->sum('integral');
        }
    ...
    public function export_order()
    {
        ...
        if ($store_name) {
            $store_id_arr = M('store')->where("store_name like '%$store_name%'")->getField('store_id', true);
            if ($store_id_arr) {
                $condition['s.store_id'] = array('in', implode(",", $store_id_arr));
            }
        }
        $condition['order_prom_type'] = array('lt', 5);
		$condition['u.user_id'] = ['neq',194836];
        $order_sn = ($keyType && $keyType == 'order_sn') ? $keywords : I('order_sn');
        $order_sn ? $condition['order_sn'] = trim($order_sn) : false;
        ...
        // }
        $orderList = M('order')
            ->field("*,FROM_UNIXTIME(add_time,'%Y-%m-%d') as create_time,s.store_name,u.mobile as store_mobile,s_order.mobile as u_mobile")
            ->join('__STORE__ s', 's.store_id= s_order.store_id')
		...
		$strTable .= '<td style="text-align:center;font-size:12px;width:180px;">三方支付单号</td>';
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">使用优惠券</td>';
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">店铺用户名</td>';
        ...
        foreach ($orderList as $k => $val) {
			...
			$strTable .= '<td style="text-align:center;font-size:12px;">&nbsp;' . $val['transaction_id'] . '</td>';
            ...
            $strTable .= '<td style="text-align:left;font-size:12px;">' . $val['u_mobile'] . '</td>';
            $strTable .= '<td style="text-align:left;font-size:12px;">' . $val['goods_price'] . '</td>';
            $strTable .= '<td style="text-align:left;font-size:12px;">' . $val['order_amount'] . '</td>';
            $strTable .= '<td style="text-align:left;font-size:12px;">' . $val['integral'].'(约'. $val['integral'] / 100 .'元)</td>'; ////
            $strTable .= '<td style="text-align:left;font-size:12px;">' . $val['platform_coupon_price'] . '</td>';
            $strTable .= '<td style="text-align:left;font-size:12px;">' . $val['pay_name'] . '</td>';
            $strTable .= '<td style="text-align:left;font-size:12px;">' . $this->pay_status[$val['pay_status']] . '</td>';
            $strTable .= '<td style="text-align:left;font-size:12px;">' . $this->shipping_status[$val['shipping_status']] . '</td>';
            $strTable .= '<td style="text-align:left;font-size:12px;">' . $val['store_name'] . '</td>';
            $strTable .= '<td style="text-align:left;font-size:12px;">' . $val['store_id'] . '</td>';
            $strTable .= '<td style="text-align:left;font-size:12px;">' . $val['store_mobile'] . '</td>';
    ...
    */
    ...
    public function order_print($order_id)
    {
       ...
       if($user_infos) $this->assign("user_infos",$user_infos);
        $order['is_drug'] == 0 ? $orderword = "订单" : $orderword = "预约";
        $order['is_drug'] == 0 ? $order_Goods = "商品" : $order_Goods = "药品";
