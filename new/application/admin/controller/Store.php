...
class Store extends Base
{
    ...
    public function store_add()
    {
        if (IS_POST) {
            ...
            $seller_name = $user_name = trim(I('user_name'));
    ...
    public function store_info_edit()
    {
        if (IS_POST) {
            ...
            $user_apply = M('store_apply')->where(array('user_id' => $store['user_id']))->count();
            if ($user_apply == 0) {
                $map['user_id'] = $store['user_id'];
                $b = M('store_apply')->add($map);
            } else {
    ...
    public function store_own_list()
    {
	    ...
	    //批量审核经营类目
    public function ajax_apply_check(){
        $bids=I('bids');
        $data['state'] =I('state');
        $map['bid'] =array('IN',$bids);
        M('store_bind_class')->where($map)->save($data);
    ...
    public function apply_class_list()
    {
        ...
        $count =M('store_bind_class')->where($where)->order('bid desc')->count();
        $Page = new Page($count, 100);
        $bind_class = M('store_bind_class')->where($where)->order('state,bid DESC')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $goods_class = M('goods_category')->getField('id,name');
    ...
    public function store_class_info()
    {
        ...
        if (IS_POST) {
            ...
            //$count = M('store_bind_class')->where($where)->count();
    ...
    public function review()
    {
        ...
        if ($data['id']) {
            ...
            if ($apply && M('store_apply')->where("id=" . $data['id'])->save($data)) {
                if($apply['seller_name']){
                    $apply['seller_name'] = trim($apply['seller_name']);
                }
