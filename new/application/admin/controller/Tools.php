...
//                    array_map("unlink", glob($path));
    ...
    public function countryHandle()
    {
        if (IS_POST) {
            $put = input('post.');
            $call_back = input('call_back');
            $data = array();
            $data['name'] = trim($put['name']);
            if (empty($put['id'])) {
                $data['create_time'] = time();
                $data['update_time'] = time();
                $name_cnt = M('country')->where('name', $data['name'])->where('status', 1)->count();
                if ($name_cnt > 0) {
                    echo "<script>parent.{$call_back}(2);</script>";
                    exit;
                }
                $rs = M('country')->add($data);
            } else {
                $name_cnt = M('country')->where('name', $data['name'])->where('status', 1)->where('id !=' . $put['id'])->count();
                if ($name_cnt > 0) {
                    echo "<script>parent.{$call_back}(2);</script>";
                    exit;
                }
                $data['update_time'] = time();
                $rs = M('country')->where('id=' . $put['id'])->save($data);
            }
            if ($rs) {
                echo "<script>parent.{$call_back}(1);</script>";
            } else {
                echo "<script>parent.{$call_back}(0);</script>";
            }
            exit();
        }
        $id = I('id');
    ...
    public function country_delete()
    {
        ...
        $rs = M('country')->where('id', $id)->setField('status', -1);
        if ($rs) {
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
