    ...
    public function detail()
    {
        ...
        if (IS_POST) {
            ...
            if ($password == '' && $password2 == '') {
                ...
                $_POST['password'] = encryptUserPasswd($_POST['password']);
            }
        ...
        if($type == 0){
            if (!empty($users)) {
                ...
                if(count($users) > 100){
                    ...
                    foreach ($suers as $k => $v) {
                        $suers[$k] = array_filter($suers[$k]);
                        // $user_list = implode(',', $suers[$k]);
                        sendNotify($suers[$k] , $tpid);
                    }
                }else{
                    $user_list=array_filter($users);
                    // $user_list=implode(',', $user_list);
                      sendNotify($user_list , $tpid);
                }    
                echo "<script>parent.{$call_back}(1);</script>";
                exit();
            }
        }else{
            $users=M('users')->where('mobile > 0')->getField('mobile');
            $ct=0;
            $suers=array();
            foreach($users as $key => $val ){
                if(count($users) > 100){
            ...
            $users_str=implode("\r\n", $users);
            if(!empty($suers)){
                foreach($suers as $ky =>$vl){
                    $tp_str=implode(',', $users[$key]);
                     $res = sendNotify($tp_str , $tpid);
                }
            }else{
              $res = sendNotify($users_str , $tpid);
            }  
            echo "<script>parent.{$call_back}(1);</script>";
            exit();
        }
