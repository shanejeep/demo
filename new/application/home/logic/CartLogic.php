...
class CartLogic extends Model
{
    ...
    function addCart($goods_id, $goods_num, $goods_spec, $session_id, $user_id = 0)
    {
        ...
        //如果是处方药添加一个字段
            if($goods['drug_attr'] == 3) $data['is_drug'] = 1;
     ...
     * @param type integer $sales_mode 销售模式
     * $mode 0  返回数组形式  1 直接返回result
     */
    function cartList($user = array(), $session_id = '', $selected = 0, $mode = 0, $sales_model = 1, $is_order = 0,$is_doc = 0,$is_drug=0)
    {
        $where = " 1 = 1 ";
        //if($selected != NULL)
        ...
        if ($sales_model != 0) {
            $where .= " and sales_model = $sales_model ";
        }
        if ($is_order == 1) {
            $where .= " and selected =1 ";
        }
        $where .=" and is_drug = ".$is_drug;
        $is_doc == 0 ? $where .= ' and doc_id = 0 ' : $where .= ' and doc_id > 0';
        $cartList = M('Cart')->where($where)->select();  // 获取购物车商品
        $anum = $total_price = $cut_fee = 0;
        $store_price = array();
        $goodsIdArr = array();
        $selectedSalesModelCnt = array(1 => 0, 2 => 0, 3 => 0);
        foreach ($cartList as $k => $val) {
            ...
            if ($selected == 1 && $val['selected'] == 0)
                continue;
            $selectedSalesModelCnt[$val['sales_model']] += 1;
            ...
            $store_price[$val['store_id']] += $val['goods_num'] * $val['member_goods_price'];
			  //获取数量
            $cartList[$k]['min_buy'] = M("goods")->where('goods_id',$val['goods_id'])->getField('min_buy');
            $cartList[$k]['max_buy'] = M("goods")->where('goods_id',$val['goods_id'])->getField('max_buy');
        }
        if ($goodsIdArr) {
            $map = array();
            $map['goods_id'] = array('in', $goodsIdArr);
            $goodsList = M('goods')->where($map)->getField('goods_id,sales_model,cat_id3', true);
            $sales_model_title = C('sales_model_title');
            $sales_model_list = array();
            foreach ($cartList as $k => $val) {
                $cartList[$k]['sales_model'] = $goodsList[$val['goods_id']]['sales_model'];
                $cartList[$k]['sales_model_title'] = $sales_model_title[$cartList[$k]['sales_model']];
                // 商品关税
                if ($cartList[$k]['sales_model'] == 3) {
                    $goodsPriceTotal = $val['goods_price'] * $val['goods_num'];
                    $cartList[$k]['custom_duty_price'] = $this->calculate_custom_duty($goodsPriceTotal, $goodsList[$val['goods_id']]['cat_id3']);
                    if ($cartList[$k]['custom_duty_price'] < 0.01) {
                        $cartList[$k]['custom_duty_price'] = '0.00';
                    }
                }
                $sales_model_list[] = $goodsList[$val['goods_id']]['sales_model'];
            }
        }
        if (!empty($sales_model_list)) $sales_model_list = array_unique($sales_model_list);
        
        $total_custom_duty_price = 0;
        foreach ($cartList as $k => $val) {
            $total_custom_duty_price += $cartList[$k]['custom_duty_price'];
        }
        //$total_price += $total_custom_duty_price;
        $total_price = array('total_fee' => $total_price, 'total_custom_duty_price' => $total_custom_duty_price, 'cut_fee' => $cut_fee, 'num' => $anum,); // 总计
        setcookie('cn', $anum, null, '/');
        if ($mode == 1) return array('cartList' => $cartList, 'selectedSalesModelCnt' => $selectedSalesModelCnt, 'total_price' => $total_price, 'sales_model_list' => $sales_model_list, 'store_price' => $store_price);
        return array('status' => 1, 'msg' => '', 'result' => array('cartList' => $cartList, 'selectedSalesModelCnt' => $selectedSalesModelCnt, 'sales_model_list' => $sales_model_list, 'total_price' => $total_price));
    }
    
    /**
     * 计算关税
     * @param $goods_price 商品价格
     * @param $cat_id3 商品三级分类
     * @return float $custom_duty 关税金额
     */
    public function calculate_custom_duty($price, $cat_id3)
    {
        $custom_duty = M('goods_category')->where('id=' . $cat_id3)->cache(true, TPSHOP_CACHE_TIME)->getField('custom_duty');
        if ($custom_duty == 0 || !$custom_duty) {
            return 0;
        }
        //return $price * ($custom_duty / 10000);
        return sprintf("%.2f", $price * ($custom_duty / 10000));
    }
    ...
    public function getCouponMoney($user_id, $coupon_id, $store_id, $mode)
    {
        ...
        $couponlist = M('CouponList')->where("(store_id = $store_id or store_id = 0) and uid = $user_id and id = $coupon_id")->find(); // 获取用户的优惠券
        if (empty($couponlist)) {
            if ($mode == 1) return 0;
            return array('status' => 1, 'msg' => '', 'result' => 0);
        }
     ...
     */
    public function addOrder($user_id, $address_id, $shipping_code, $invoice_title, $coupon_id = array(), $car_price,$params = array())
    {
        ...
        // 循环添加订单 多少个商家添加多少个订单
		 /***********dengxing begin*********/
        $get_pcommis_uid = M('users_reg_relation')->where(array('user_id'=>$user_id,'sendUserType'=>1))->getField('relation_user_id');
        if(empty($get_pcommis_uid)){ //没有推广佣金获得者 默认为13364031752
            $get_pcommis_uid = M('users')->where(array('mobile'=>'13364031752'))->getField('user_id');
        }
        /***********dengxing end*********/
        ...
        foreach ($car_price['store_order_amount'] as $k => $v) {
		 ...
		 //-----------deng start--------------
            ...
            if ($car_price['store_point_count'][$k] > 0) {
                $integral_money = ($car_price['store_point_count'][$k] / tpCache('shopping.point_rate'));
            }
            //$commission=serialize(array(session('doctor_id') => $car_price['yj_money'][$k]));
            ...
            $data = array(
                ...
                'true_name' => $params['user_true']['true_name'], //'发票抬头',
                'id_card' => $params['user_true']['id_card'], //'发票抬头',
                'user_note' => $params['user_note'][$k], //'给卖家留言',
                ...
                'platform_coupon_price' => $car_price['platform_balance'][$k], // 商家平摊平台优惠券金额
                ...
                'add_time' => time(), // 下单时间
                'custom_duty_price' => $car_price['store_custom_duty_price'][$k],// 订单关税,
                'order_prom_id' => $car_price['store_order_prom_id'][$k],//'订单优惠活动id',
                'order_prom_type' => $store_order_prom_type = empty($car_price['store_order_prom_type'][$k]) ? 0 : $car_price['store_order_prom_type'][$k],//'订单优惠活动id',
                'order_prom_amount' => $car_price['store_order_prom_amount'][$k],//'订单优惠活动优惠了多少钱',
                'store_id' => $k,  // 店铺id
                'sales_model' => $car_price['sales_model'],
                'promote_commission' => $car_price['promote_commission_money'][$k],// 推广佣金 dengxing
                'sales_commission' => $car_price['sales_commission_money'][$k],// 销售佣金 dengxing
                'get_pcommis_uid' => $get_pcommis_uid,//推广佣金获得者 dengxing
                'get_scommis_uid' => $doctor_id,//销售佣金获得者 dengxing
                'info_id' => $params['info_id'], //用药人id
                'is_drug' => $params['is_drug'], //是否处方药
                'case1' => $params['case1'], //病例1 2 3 4
                'case2' => $params['case2'],
                'case3' => $params['case3'],
                'case4' => $params['case4'],
            );
            ...
            if (!$order_id) return array('status' => -9, 'msg' => '下单失败', 'result' => '');
            ...
            // 1插入order_goods 表
            $cartList = M('Cart')->where("store_id = $k and user_id = $user_id and is_drug= {$params['is_drug']} and selected = 1 and sales_model={$car_price['sales_model']}")->select();
            //分销购物车取的session
            if ($params['immed_buy'] == 2) {
                $cartList = [];
                $cart_info = session('cart_info');
                $cartList[] = unserialize($cart_info);
            }
            foreach ($cartList as $key => $val) {
                ...
                $custom_duty_price = 0;
                if ($goods['sales_model'] == 3) {
                    $custom_duty_price = $this->calculate_custom_duty($val['goods_num'] * $val['goods_price'], $goods['cat_id3']);
                }
                $data2['sales_model'] = $goods['sales_model']; // 销售模式
                $data2['custom_duty_price'] = $custom_duty_price; // 商品关税
            ...
            // 4 清空购物车
            if($params['immed_buy'] == 1){
                M('Cart')->where("store_id = $k and is_drug={$params['is_drug']} and user_id = $user_id and selected = 1 and sales_model= {$car_price['sales_model']}")->delete();
            }
            ...
            if ($user['oauth'] == 'weixin') {
               ...
               $wx_content = "您刚刚提交了一笔订单：{$order['order_sn']}，别忘记支付了哦，它已经迫不及待地想与你见面~";
     ...
     * $mode 0  返回数组形式  1 直接返回result
     */
    public function cart_count($user_id, $mode = 0, $sales_model_id = 0)
    {
        $where = "user_id = $user_id and selected = 1 ";
        if ($sales_model_id != 0) {
            $where .= "and sales_model= $sales_model_id";
        }
    ...
    public function login_cart_handle($session_id, $user_id)
    {
        ...
        M('cart')->where("session_id = '$session_id' and user_id = 0")->save(array('user_id' => $user_id));
     ...
     * $mode 0  返回数组形式  1 直接返回result
     */
    function cartInfoList($user = array(), $session_id = '', $selected = 0, $mode = 0, $sales_model = 1, $is_order = 0)
    {
        $where = " 1 = 1 ";
        //if($selected != NULL)
        //    $where = " selected = $selected "; // 购物车选中状态
        ...
        $cartList[0] = unserialize($cartListinfo);
        if ($cartList == false) { //session存储失败
            return false;
        }
        $anum = $total_price = $cut_fee = 0;
        $store_price = array();
