...
class GoodsLogic extends Model
{
    ...
    // 通过goods_id 获取 商品类别ID
    public function cat_id_get_goods_ids($cat_id)
    {
        if (!$cat_id) return array();
        return M('goods_category_data')->where('cat_id', $cat_id)->cache(GOODS_CACHE_TIME)->getField('goods_id', true);
...
//        $c3 = M('Comment')->where("goods_id = $goods_id and parent_id = 0 and  ceil(goods_rank) in(1,2)")->count(); // 差评
     ...
     * @return type
     */
    public function collect_goods_bck($user_id, $goods_id)
    {
    ...
    public function collect_goods($user_id, $goods_id)
    {
        ...
        if ($count > 0) {
            ...
            Db::name('goods_collect')->where("user_id", $user_id)->where("goods_id", $goods_id)->delete();
            return array('status' => -2, 'msg' => '收藏已取消', 'result' => array());
        } else {
            Db::name('goods')->where('goods_id', $goods_id)->setInc('collect_sum');
            Db::name('goods_collect')->add(array('goods_id' => $goods_id, 'user_id' => $user_id, 'add_time' => time()));
            return array('status' => 1, 'msg' => '收藏成功!请到个人中心查看', 'result' => array());
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            if ($i == 0)
                $parr[] = array('value' => "1 - {$end}", 'href' => urldecode(U("Goods/$action", $filter_param, '')));
            elseif ($i == ($c - 1))
    ...
    function getGoodsDispatching($goods_id, $region_id)
    {
        ...
        if (empty($goods['shipping_area_ids'])) {
            ...
            foreach ($plugin_goods_shipping as $k => $v) {
                $goods_shipping[$k]['freight'] = $cart_logic->cart_freight2($plugin_goods_shipping[$k]['code'], 0, 0, 0, $goods['weight'], $goods['store_id']);//默认全国
