...
class Goods extends MobileBase
{
    ...
    public function goodsInfo()
    {
        ...
        $from = I("from",0); //是否从app首页直接点进商品 0:商城进入 1:app首页进入
        ...
        $goods['sale_num'] += $goods['sale_count'];
...
//        $show = $page->show();
        ...
        foreach ($list as $k => $v) {
            ...
            $list[$k]['mobile'] = substr_replace($v['mobile'],'****',3,4); // 晒单图片
        }
        $goodsLogic = new \app\home\logic\GoodsLogic();
        $commentStatistics = $goodsLogic->commentStatistics($goods_id);// 获取某个商品的评论统计
        $this->assign('commentStatistics', $commentStatistics);//评论概览
    ...
    public function claimGoods()
    {
        ...
        if (!isset($_GET['is_have'])) {
            $_GET['is_have'] = 0;
        }
        $is_have = I('get.is_have', '0'); // 0所有商品 1已收藏商品
        $this->assign('is_have', $is_have);
        $doctor_id = I('get.doctor_id', ''); // 当前医生id 发送人  环信会员用户名
        $member_id = I('get.member_id', ''); // 信息接收人id  环信会员用户名
        $this->assign('doctor_id', $doctor_id);
        $this->assign('member_id', $member_id);
        $doctor_id =M('users')->where('mobile',$doctor_id)->getField('user_id');
        $member_id =M('users')->where('mobile',$member_id)->getField('user_id');
        $this->assign('doctor_id', $doctor_id);
        $this->assign('member_id', $member_id);
        ...
        if ($goodsCate) {
            ...
            if ($is_have > 0) { //收藏列表
                ...
                $filter_goods_id = M('goods g')->where(" g.goods_state = 1 and g.is_on_sale=1 and cr.user_id={$doctor_id} and (g.sales_commission > 0 or g.promote_commission >0) and g.cat_id{$goodsCate['level']} = $id")->join('claim_record cr', 'cr.goods_id=g.goods_id', 'right')->getField("g.goods_id", true);
            }
            /*-------------- deng start ------------*/
            else{
        ...
        if ($count > 0) {
            ...
            if ($is_have > 0) $goods_list_where .= "and cr.user_id = {$doctor_id} ";//收藏列表
            $q = input('q','');
    ...
    public function assemblyInfo()
    {
        ...
        $goods=M('goods')->where(array('goods_id'=>$goods_id))->find();
        if(!empty($goods)){
            $head_url='http://'.$_SERVER['HTTP_HOST'].'/mobile/goods/claimGoodsInfo/id/'.$goods_id.'/doctor_id/'.$doctor_id.'/member_id/'.$member_id;
    ...
    public function claimGoodsInfo()
    {
        ...
        $doctor_id = I('get.doctor_id', ''); // 当前医生id
        $member_id = I('get.member_id', ''); // 当前会员id
        $mb_type = I('t','');  //分享通过web进入的

		if($this->app_type == 3 || $this->app_type == 4){
			 $this->redirect(url('Mobile/Goods/goodsInfo', array('id' => $goods_id,'from'=>1)));
		}
        //如果为1测不需要登录，是分享出来的
        if(empty($mb_type)){
            if ($doctor_id == '' || $member_id == '') {
