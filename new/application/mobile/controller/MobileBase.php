...
class MobileBase extends Controller
{
    ...
    public $access_type = 0; // 访问类型 app
    public $app_type = 0; // 1 IOS 患者端 2 android 患者端 3 IOS 医生端 4 android 患者端
    public $login_url;
    public $url_type;
    ...
    {
        // 访问方式处理（手机app webview 、手机浏览器）
        $this->access_type = I('access_type') ? I('access_type') : Cookie::get('access_type');
        $this->app_type = I('app_type') ? I('app_type') : Cookie::get('app_type');
		$this->assign("httpHosts","http://".$_SERVER['HTTP_HOST']);
        $this->assign('app_type', $this->app_type);
        $this->url_type = I('access_type', 0);
        $this->assign('url_type', $this->url_type);
        Cookie::set('access_type', $this->access_type);
        Cookie::set('app_type', $this->app_type);
        $this->login_url = MobileLoginUrl($this->access_type, $this->app_type);
        $this->assign('login_url', $this->login_url);
        $this->assign('access_type', $this->access_type);
        $this->token = I('token') ? I('token') : cookie('YZJSESSID');

        if ($this->access_type == 1) { // app webview 范围
            if (!$this->token) {
                exit('参数错误！');
            }
            // 设置session_id
            if ($this->session_id != $this->token) {
                $this->session_id = session_id($this->token);
            }
            // 获取用户信息
            $userLoginRes = AppUser::getInstance()->setRedis(0)->setToken($this->token)->isLogin();
            $app_user = ($userLoginRes) ? AppUser::getInstance()->getUserInfo() : array();
            ...
            // 第一次访问
            if ($app_user['user']['phone']) {
                $this->userInfo = M('users')->where('mobile=' . $app_user['user']['phone'])->find();
                if(empty($this->userInfo)){
                    $mylogic = new \app\home\logic\UsersLogic();
                    $res_info = $mylogic->reg($app_user['user']['phone'],'12345678a','12345678a',$app_user['user']['uid'],$app_user['user']['type']);
                    $this->userInfo = $res_info['result'];
                }
            } else { // 多次访问
                $this->userInfo = $app_user['user'];
            }
            if (!$this->userInfo) {
                header("location:" . $this->login_url);
                exit;
            ...
            if ($this->token) {
                ...
                // 获取用户信息
                $userLoginRes = AppUser::getInstance()->setRedis(0)->setToken($this->token)->isLogin();
                $this->userInfo = ($userLoginRes) ? AppUser::getInstance()->getUserInfo() : array();
            }
        }
        ...
        if ($this->access_type == 1) {
            $nickname = !empty($this->userInfo['nickname']) ? $this->userInfo['nickname'] : $this->userInfo['mobile'];
            session('user', $this->userInfo);
            setcookie('user_id', $this->userInfo['user_id'], null, '/');
            setcookie('uname', $nickname, null, '/');
            setcookie('cn', 0, time() - 3600, '/');
            setcookie('is_distribut', $this->userInfo['is_distribut'], null, '/');
        }
        // 判断当前用户是否手机
        if (isMobile())
    ...
    {
        // IOS
        if (in_array($this->app_type, array(1, 3))) {
            $page = 'return_app_ios';
        } else { // android
            $page = 'return_app_android';
        }
