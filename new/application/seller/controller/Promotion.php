...
class Promotion extends Base
{
    ...
    public function prom_order_save()
    {
        ...
        if(array_key_exists('money',$data) && array_key_exists('expression',$data)){
            if($data['expression'] >= $data['money'] && $data['type'] > 0){
                $this->error('立减金额不能大于等于需要满足的金额');
            }
        }
        if ($prom_id) {
