...
class Report extends Base{
	...
	public function _initialize(){
		...
		if($timegap){
			...
			$lastweek = date('Y-m-d',strtotime("-1 week"));//30天前
	...
	public function index(){
		...
		$today['today_amount'] = M('order')->where("add_time>$now AND (pay_status=1 or pay_code='cod') and store_id=$this->store_id and order_status in(0,1,2,4)")->sum('order_amount');//今日销售总额
		...
		$sql = "SELECT COUNT(*) as tnum,sum(order_amount) as amount, FROM_UNIXTIME(add_time,'%Y-%m-%d') as gap,add_time,sum(user_money) as umoney from  __PREFIX__order ";
		$sql .= " where add_time>$this->begin and add_time<$this->end and store_id=$this->store_id AND pay_status=1 and order_status in(0,1,2,4) group by gap  order by gap DESC";
		$res = Db::query($sql);//订单数,交易额
		
		foreach ($res as $val){
			$arr[$val['gap']] = $val['tnum'];
			$brr[$val['gap']] = $val['amount'] + $val['umoney'];
			$tnum += $val['tnum'];
			$tamount += $val['amount'];
		}
