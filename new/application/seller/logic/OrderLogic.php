...
class OrderLogic extends model
{
    ...
    public function getOrderGoods($order_id){
        ...
        $salesModelTitleArr = C('sales_model_title');
        foreach ($res as $key=>&$val){
            $val['sales_model_title'] = $salesModelTitleArr[$val['sales_model']];
        }
        return $res;
    }
...
//        	$btn['remove'] = '移除';
    ...
    public function deliveryHandle($data,$store_id){
        ...
        switch ($data['shipping_code']) {
            ...
            default:
                $params = array($data['consignee'], $order['shipping_name'],$data['invoice_no']);
                break;
        }
        send_notify_sms("5", $data['mobile'], $params,$tpid);
		
		// $params = array($data['consignee'], $order['shipping_name'],$data['invoice_no']);
        // send_notify_sms("5", $data['mobile'], $params,227407);
