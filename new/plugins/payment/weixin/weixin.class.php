...
{    
    ...
    function getJSAPI($order){
		  if(stripos($order['order_sn'],'recharge') !== false){
			...
			$go_url = U('Mobile/User/order_detail',array('id'=>$order['order_id'],'ps'=>1));
    		$back_url = U('Mobile/Cart/cart4',array('order_id'=>$order['order_id']));
    	}
        if(!is_weixin()){
			require_once("wechatH5Pay.php");
            $appid  = WxPayConfig::$appid;                  //应用APPID
            $mch_id = WxPayConfig::$mchid;                  //微信支付商户号
            $key    = WxPayConfig::$key;
            $notify_url = SITE_URL . '/index.php/Mobile/Payment/notifyUrl/pay_code/weixin';//回调地址
            $wechatAppPay = new \wechatAppPay($appid, $mch_id, $notify_url, $key);
            $params['body'] = "支付订单：" . $order['order_sn'];                       //商品描述
            $params['out_trade_no'] = $order['order_sn'] . time();    //自定义的订单号
            $params['total_fee'] = $order['order_amount'] * 100;                       //订单金额 只能为整数 单位为分
            $params['trade_type'] = 'MWEB';                   //交易类型 JSAPI | NATIVE | APP | WAP 
            $params['scene_info'] = '{"h5_info": {"type":"Wap","wap_url": "http://shop.yzjia.com","wap_name": "药之佳商城"}}';
            $result = $wechatAppPay->unifiedOrder( $params );
            $url = $result['mweb_url'].'&redirect_url='.SITE_URL . $go_url."/ps/1";//redirect_url 是支付完成后返回的页面
            $html= <<<EOF
