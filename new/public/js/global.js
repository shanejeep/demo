...
function get_city(t,selected){
    var parent_id = $(t).val();
    if(parent_id == '0'){
        var city_html = '<option value="0">选择城市</option>';
        $('#city').empty().html(city_html);

        var district_html = '<option value="0">选择区域</option>';
        $('#district').empty().html(district_html);
 ...
 * 获取多级联动的商品分类
 */
function get_category(id,next,select_id,is_drug=0){
    var url = '/index.php?m=Home&c=api&a=get_category&is_drug='+is_drug+'&parent_id='+ id;
    $.ajax({
        type : "GET",
        url  : url,
        error: function(request) {
