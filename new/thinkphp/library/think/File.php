...
class File extends SplFileObject
{
    ...
    public function fileup($info,$dirs,$oldurl=null){
		$oss_config = Config::get("aliyun_oss");
        $bucket = $oss_config['bucket'];
        $oss = $oss_config['ossurl'];
        Vendor('.aliyuncs.oss-sdk-php.autoload');
        $ossClient= new OssClient($oss_config['accessKeyId'],$oss_config['accessKeySecret'],$oss_config['endpoint'],false);

        //判断bucketname是否存在，不存在就去创建
        if( !$ossClient->doesBucketExist($bucket)){
            $ossClient->createBucket($bucket);
        }
        $nameArr=explode('.', $info->info['name']);
        $nametype=end($nameArr);
        switch($dirs)
        {
            case 'store/cert':
                $object = 'upload/'.$dirs.'/'.date('Y').'/'.date('m-d').'/'.md5(microtime(true)).'.'.$nametype;//想要保存文件的名称  
                break;
            default:
                $object = 'upload/'.$dirs.'/'.date('Y').'/'.date('m-d').'/'.md5(microtime(true)).'.'.$nametype;//想要保存文件的名称  
        }
        ...
        $file=$info->info['tmp_name'];
        $ossClient->uploadFile($bucket,$object,$file);
        //删除旧图片
        if(!empty($oldurl)){
            if(is_array($oldurl)){
                foreach ($oldurl as $k => $v) {
                    $oldurl[$k]=str_replace($oss, '',$v);
                }
                  $ossClient->deleteObjects($bucket, $oldurl);
            }else{
                  $oldurl=str_replace($oss, '',$oldurl);
                  $ossClient->deleteObject($bucket, $oldurl);
            }
        }
